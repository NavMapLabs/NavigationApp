...
import { View, StyleSheet, Pressable, Text, FlatList, TextInput, NativeSyntheticEvent, TextInputKeyPressEventData } from 'react-native';
import { IconButton } from 'react-native-paper';
import { SearchHistoryNavigationProp } from '@/constants/types';
import { useNavigation } from '@react-navigation/native';
import { search, getMetaDataById, map_meta, getDataById} from '@/scripts/BackendFunc';
import { loadMapState, NavMapState } from '@/store/NavMapSlice';
import { useDispatch } from 'react-redux';
import { AppDispatch } from '@/store/datastore';
import { loadMapMeta } from '@/store/NavStateSlice';
...
const SearchBar = ({ searchQuery, setSearchQuery, setFiles }) => {
    const onKeyDownHandler = async (e: NativeSyntheticEvent<TextInputKeyPressEventData>) => {
        if (e.nativeEvent.key === 'Enter') {
            // Perform the action when "Enter" is pressed
            console.log("enter pressed:" + searchQuery)
            const files: { name: string; owner: string; updated: string; id: string }[] = await search(searchQuery)
            console.log(files)
            // const file = [
...
const FileDisplay = ({ searchQuery, files }) => {
    ...
    const open_map = async (item : { name: string; owner: string; updated: string; id: string } ) => {
        console.log(item.id)
        const metaData: map_meta | undefined = await getMetaDataById(item.id)
        if(metaData){
            console.log(metaData.mapName)
            dispatch(loadMapMeta({meta_info: metaData}))
        }
        const mapData:NavMapState | undefined = await getDataById(item.id)
        if (mapData) {     
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <View style={[styles.listRow, styles.boxFiles]}>
                        {/* Pressable for name */}
                        <Pressable style={{ width: '33%' }} onPress={() => {open_map(item)}}>
...
const SearchHistory = () => {
    ...
    return (
        <View style={styles.container}>
            ...
            <SearchBar searchQuery={searchQuery} setSearchQuery={setSearchQuery} setFiles = {setFiles} />
            {/* Passing searchQuery to FileDisplay Function */}
            <FileDisplay searchQuery={searchQuery} files = {files}/>
        </View>
    );
};
